...
Whenever you hear something about Haskell, chances are it sounds arcane and
...
unnecessary, if the concept of functors is explained using a simple analogy.

<!-- more -->
If you have a situation where a type contains other types, you are dealing with
...
but are not central to how one should think about functors.

##In practice
At a GHCi prompt, you can import a few modules here and there with the word
...
If you are familiar with Haskell syntax, you should know that this means that
'Just' takes any type and turns it into a 'Maybe' functor containing that type.
No surprises here, that is the definition of a functor.

Let's say we have a few functions that return numbers contained in a Maybe
functor, and we want to combine them in some way. When mapping over a single
functor, the function you use is fmap, like so:

<!--```haskell-->
<pre><code>Prelude> fmap (+2) (Just 3)
Just 5
</code></pre>

This is fine for function that only take 1 argument, but remember, we want to
...
fmap on its own won't work:

<!--```haskell-->
<pre><code>Prelude> let f x y z = x*y*z
</code></pre>

If we just try to fmap our three-operand function over a Maybe functor, and
examine its type, we see the following:

<!--```haskell-->
<pre><code>Prelude> :t (fmap (f) (Just 3))
(fmap (f) (Just 3)) :: Num a => Maybe (a -> a -> a)
</code></pre>

This means that fmap has transformed our function which took things of typeclass
...
multiple arguments to functors, we need to use Applicative functors.

<!--```haskell-->
<pre><code>Prelude> import Control.Applicative
</code></pre>

And then, we can use the `<*>` operator, which takes a function in a functor and
applies it to a value in a functor. We can use it in more trivial cases like so:

<!--```haskell-->
<pre><code>Prelude> Just (*4) <*> Just 3
Just 12
</code></pre>

We can see that `(*4)` takes one value, so we are given the answer we want.
Applying this to our situation gives us this:

<!--```haskell-->
<pre><code>Prelude> (fmap (f) (Just 3)) <*> Just 4 <*> Just 5
Just 60
</code></pre>

We can improve the look of this ugly statement by using fmap as an infix
function.

<!--```haskell-->
<pre><code>Prelude> f `fmap` Just 3 <*> Just 4 <*> Just 5
</code></pre>

Because this is such a common use case, there is some syntactic sugar provided -
fmap can be replaced with `<$>`.

<!--```haskell-->
<pre><code>Prelude> f <$> Just 3 <*> Just 4 <*> Just 5
</code></pre>

At this point, we just have the function and its arguments as we usually would,
...
but thinking about it like a monad rarely helps.
