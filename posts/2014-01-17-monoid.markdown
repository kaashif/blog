---
title: What's a monoid?
comment: Category theory: a highly mathematical and abstract field of study that has wide implications in everyday scenarios.
date: 2014-01-14
---


If you have spent time on programming or technology boards like /g/ or
...
from scary functional languages like Haskell. The truth is that monads aren't
even slightly the same sort of thing in Haskell, but they do relate somewhat.
Monoids are, in actual fact, very, very simple.

##Sets and operators
Let's take an example: suppose you have a set of things _S_ and a single binary
...
We could make this an even more relatable example by using the set of real
...
This means that whenever you're adding real numbers (which we all do on a daily
...
While it is all well and good having a single set and a single binary operator,
...
In some cases, rather than having sets of sets within our structure, we may want
...
objects. So now, our structure consists of:

+ A set of objects
+ A set of morphisms mapping objects to other objects

This structure is not all that different to a monoid, and it becomes very
obvious if we say a monoid merely consists of:
...
Since there is only one object, the morphism can only map the object to itself,
...
see the monoid as a set _S_, and a set of unary functions equal in order to _S_.
We get the same result whether we think about a single endomorphism or multiple,
so it doesn't really matter. 

What we are slowly working towards, as our definitions get progressively more
general, is a category. Categories are incredibly useful and can become so
general that they are used to formalise _all_ of mathematics. They actually have
uses too, such as considering the objects as representing types in a programming
language, and the morphisms as representing functions.
