...
comment: Scripting DNS record updates with Perl. Useful if you like knowing exactly what, when and how your dynamic DNS is being updated.
date: 2014-01-22
---


When running a website on a residential connection, a problem one might run into
...
it changes. At one time, your IP might be 10.0.0.1, and your domain correctly
resolves to 10.0.0.1, but after you reboot your modem or router, your IP may
...
Assuming that getting a static IP is impossible, there is only one solution:
...
my domain with Namecheap, which is where my domain was registered.

##Choosing a client
Several websites, DynDNS and NoIP being examples, offer their own clients. You
...
Most services provide a URL with which you can update your A records. Fetch this
...
default on most GNU/Linux distros.

##Writing the client
There is some Perl boilerplate we have to get out of the way - the shebang and
some "use" statements.
...
It's worth commenting on my use of "env". On FreeBSD (the OS my servers use),
...
Next, we have to find the URL, password, hosts, and domain we will use. Namecheap uses
...
Notice we didn't attempt to put the hosts into the initial definition of the
update URL. We will do that when we loop over the array and fetch the URL using
cURL, thus updating the domain.
...
So far, we have fetched the URL, so the IP of the host's A record should be
...
It's possible to use XML::Simple and actually parse the XML output by the server
...
not even you, will ever care (unless you just do it for practice).

So save this script in /usr/local/bin/dnsup, or somewhere else appropriate and
memorable.

##Scheduling updates
Your IP address can change instantly at any moment, so you must balance the
...
I run it as "nobody" to avoid any unnecessary root usage. What if you made a
mistake in the script and overwrote / with garbage? A program having privileges
is bad, don't do it.

At this point, you should be free of any possibly proprietary, bloated DNS
clients and you should have an automatically updating domain.
