...
comment: Muon: the static site generator I wrote because none of the current ones work in the way I'd like. How to get it and how to use it.
date: 2014-04-13
---

UPDATE: I now use Hakyll. See <http://kaashif.co.uk/about> for
more. Also, 100% of the information in this blog post is now wrong or outdated.

When I decided I wanted to write a blog, I had to come up with some way
of writing posts (in a markup format which isn't HTML), and serving them
somehow.

First, I turned to Jekyll, a static blog generator written in Ruby and
...
getting bored of a solution which worked _too_ well to be any fun.

Obviously, I had the right idea in the first place, with generating
...
At some point, when I consider Muon to be ready for use by normal
people, or at least, as normal as a Haskell package user can be, I will
upload it to [Hackage](http://hackage.haskell.org) and you'll be able to
install it with a simple:

    $ cabal install muon

As it is, you have to get the latest snapshot from my darcs repository.
...
It might take a while to build the dependencies from source, so if your
OS's package manager already has prebuilt binary packages for them, look
inside muon.cabal to see what you need to install, and install those.

##Using Muon

If you want a quick rundown of what you can do with Muon, have a look at
the README file in the darcs repo. If you're too lazy for that, just
access Muon's help:

    $ muon help

In reality, it doesn't matter what you type as the command - muon
outputs a help message for all inputs that aren't already a command. So
that's everything other than "generate", "init" and "upload".

It's worth noting that, at the moment, my server is hard coded into
...
Before writing Muon, I had the idea in my head that Haskell was useless
...
There is always this talk of explaining monads using analogies and shit
...
analogy, if it does, I'm a complete hypocrite.

Anyway, Haskell isn't too hard to learn. Neither is Common Lisp. It gets
...
constructs without much effort.
