---
title: Patching OpenBSD
comment: Checking out the latest sources, writing a patch and getting it accepted: the ins and outs of contributing to OpenBSD.
date: 2014-08-24
---


Recently, I've been trying to understand the ins and outs of CVS in
...
abilities until recently.

##How does OpenBSD's contributing system work?

OpenBSD uses a tried and tested method of accepting contributions from
...
From here on out, I'll assume you're trying to update a program in the
...
by Emacs and WordStar (yeah, it's not the _newest_ editor either).

The first step is finding a nearby CVS repo. A good place to look is on
[this list](http://www.openbsd.org/anoncvs.html#CVSROOT). Pick one
...
server in Europe and export CVSROOT:
...
Now you have a fully updated ports tree in /usr/ports! Note that I
didn't specify any flags: on OpenBSD, there is a default ~/.cvsrc that
comes by default in every user's home. Mine looks like this:
...
Now, you might want to find the file you want to change. In this
example, it's in editors/joe, so lets go there in the ports tree:

    # cd /usr/ports/editors/joe

I'm not going to explain how to edit Makefiles and update ports in
OpenBSD, a guide for that already exists
[here](http://www.openbsd.org/faq/ports/guide.html). Instead, lets
...
If you delete any files or add any files, you have to do `cvs delete
<file>` or `cvs add <file>` for the changes to be tracked properly. Do
this before making a patch in the next step.

Now you have to get the changes you just made into an email, which is
easy using `cvs diff`. Run `cvs diff` and redirect its output to a file
somewhere.

    # cvs diff > /tmp/joe.diff

If you look at /tmp/joe.diff, you'll see that all the changes you made
...
running `patch -p0 < /tmp/joe.diff` in the right directory
(/usr/ports/editors/joe, in this case).

##Mailing that patch to ports@

In case you don't know how mailing lists work, let me explain. You send
...
list, who can then apply the patch.

In your email client, write a new message with the subject "UPDATE: joe
...
In the rest of the message, paste your diff. It's easier for everyone if
you just put your diff in the body of the message, because they can then
just apply your email as the patch and everything will work out fine.

Now send the mail and wait.

If you get a response, take into consideration any criticism and send a
...
commit it to CVS and you'll have contributed to OpenBSD!
