...
Back in the day, I used to use Slackware. It was the best distro
...
that popular. It's still a solid distro, though. There is one problem
<!-- more -->
...
Maybe it is, but it causes a lot more problems than it solves. If
...
much cooler selling point: USE flags).

Using binary packages without dependency management just causes
...
Of course, there is a "solution" in place: queues. You're supposed to
look up the whole dependency tree beforehand and queue up all of the
packages, one by one. Pretty tedious.

There are alternatives, like slapt-get, but those require you to
...
control. Where can we turn? Of course, NetBSD's pkgsrc.

##Getting started

You might think that since it's part of the NetBSD project, pkgsrc can
...
The need for "a" is obvious: that's the base. You need "d" and "l" for
...
tree. You can't do that yet, though.
...
Now, there is a ports-looking sort of thing under
/usr/pkgsrc. Nifty. But you need NetBSD's make to use it. Luckily,
there's a bootstrap script to get you started.

<pre><code># cd /usr/pkgsrc/bootstrap
# ./bootstrap
</code></pre>

Something should happen and at the end, there should be some binaries
...
<pre><code>export PATH=$PATH:/usr/pkg/bin:/usr/pkg/sbin
</code></pre>

Additionally, you might want man pages.

<pre><code>export MANPATH=$MANPATH:/usr/pkg/man
</code></pre>

Now everything's set up to start building. You might not have an X
server yet and even if you do, you'll want to replace it with
pkgsrc's. Installing a port is easy:

<pre><code>$ cd /usr/pkgsrc/x11/modular-xorg-server
$ sudo bmake install
</code></pre>

On my old laptop, this took several hours. If you have a more recent
machine, it shouldn't take too long.

While building, you might think "Hey, why's it installing Perl, I
...
package database, which is completely empty.

This means that you'll end up building Perl, Python and dozens of
...
/var/log/packages and and `removepkg` everything you don't want.

##What now?

You can search the ports tree with /usr/pkgsrc/pkglocate for all your
...
Disclaimer: I do not use pkgsrc on Slackware on a day-to-day basis, so
don't blame me for your problems. Send something in to the pkgsrc
mailing list or some NetBSD IRC channel instead.
