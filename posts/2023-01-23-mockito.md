...
Which is pretty cool, even if it’s a bit magic. There are a lot of blog posts out there about how this works and what proxy-based mocking libraries do, for example: <http://blog.rseiler.at/2014/06/explanation-how-proxy-based-mock.html>

If you have a good understanding of Mockito, there’s no need to read that post but otherwise I recommend it. The summary is that Mockito intercepts method calls and keeps track of which methods have been called globally, and with what arguments. The call to `.thenReturn` effectively writes to global state, so that the next call to `mockObject.myMethod(1)` will have the right behaviour.
